...
_ARRIVAL_RE      = re.compile(r”^\s*ARRIVAL\s*:”,                  re.IGNORECASE)
...
def mark_bold_in_block(block_lines):
“””
Trả về list (line, bold) cho từng dòng trong block.
Tất cả dòng từ FLIGHT … đến ARRIVAL … (kể cả 2 dòng đó) đều in đậm.
“””
result = []
...
for line in block_lines:
    if _FLIGHT_START_RE.match(line):
        in_flight_section = True
        result.append((line, True))
    elif _ARRIVAL_RE.match(line):
        result.append((line, True))
        in_flight_section = False   # sau ARRIVAL thì thôi đậm
    elif in_flight_section:
        result.append((line, True))
    else:
        result.append((line, is_bold(line)))
...
```
# Logo — thu nhỏ, không quá rộng so với nội dung bên dưới
LOGO_MAX_W = CONTENT_W * 0.40   # tối đa 40% chiều rộng content
...
# Header — in đậm tên hành khách và Booking Ref; bỏ "xx/" trước mã
for line in header_lines:
    # Bỏ prefix "xx/" hoặc số dạng "12/" trước mã booking
    cleaned = re.sub(r'(?i)(BOOKING\s+REF\s*:\s*)\w+/', r'\1', line)
    # In đậm nếu là tên hành khách (dạng HO/TEN) hoặc BOOKING REF
    bold_hdr = bool(
        re.search(r'BOOKING\s+REF\s*:', cleaned, re.IGNORECASE) or
        re.match(r'^[A-Z\-]+/[A-Z]', cleaned.strip())
    )
    story.append(MonoLine(cleaned, bold=bold_hdr))
...
for block_idx, block in enumerate(blocks):
    elements = []
    for line, bold in mark_bold_in_block(block):
