...
def **init**(self, text, bold=False, red=False):
super().**init**()
self.text   = text
self.bold   = bold
self.red    = red
    ...
    for line in block:
        ...
        if re.match(r"FLIGHT\s+TICKET\(S\)", s, re.IGNORECASE) or \
           ...
           re.match(r"[A-Z]{2}/ETKT\s", s, re.IGNORECASE):
            ...
            in_flight_section = False  # ticket section — kết thúc vùng flight
        ...
        # Kết thúc vùng in đậm khi qua ARRIVAL
